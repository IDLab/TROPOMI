...
fCollectSingleShipObs <- function (NO2Obs, Shipdata, ShipName) {
  #ShipName <- ShipNames[12]
  
  SingleShipLoc <- filter(ShipData, Name == ShipName)
  nrow(SingleShipLoc)
  
  #extract nearest neighbour TROPOMI observations of known AIS locations
  ...
  #calculate a normalized bearing as to orientate axis default as coming from the North
  SingleShipSurround$normBearing <- SingleShipSurround$Bearing-bestSingleShipLoc$Origin
  SingleShipSurround$normBearing[SingleShipSurround$normBearing < 0] <- SingleShipSurround$normBearing[SingleShipSurround$normBearing < 0] + 360
  ...
  DistanceSectors <- seq(min(SingleShipSurround$DistToOrg),max(SingleShipSurround$DistToOrg), length.out = 8)
  ...
  ##########
  #Plot a radial and sector plot of point cloud
  ...
  ggplot(RadialPlotData, aes(fill = meanDeltaConc, xmin =PolarSector, ymin = DistanceSector, xmax = PolarSector+360/16, ymax = DistanceSector+1)) +
    geom_rect() +
